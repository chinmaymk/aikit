  ...
  describe('generate', () => {
    ...
    it('should call OpenAI API with correct parameters', async () => {
      ...
      const scope = mockChatCompletion(
        [chatTextChunk('Hello!'), chatStopChunk()],
    ...
    it('should handle multimodal content with images', async () => {
      ...
      let requestBody: any;
      const scope = mockChatCompletion(
        [chatTextChunk('I see an image', 'stop')],
        body => (requestBody = body)
      );
    ...
    it('should handle assistant messages with tool calls', async () => {
      ...
      let requestBody: any;
      const scope = mockChatCompletion(
        [chatTextChunk('Let me check...', 'stop')],
        body => (requestBody = body)
      );
    ...
    it('should handle tool result messages', async () => {
      ...
      let requestBody: any;
      const scope = mockChatCompletion(
        [chatTextChunk('The weather is sunny', 'stop')],
        body => (requestBody = body)
      );
    ...
    it('should process streaming response correctly', async () => {
      const scope = mockChatCompletion(
        [chatTextChunk('Hello'), chatTextChunk(' there'), chatTextChunk('!'), chatStopChunk()],
        () => {}
      );
    ...
    it('should handle malformed JSON in tool arguments', async () => {
      const scope = mockChatCompletion(
        [
          ...
          chatToolCallChunk({ index: 0, args: '}' }, 'tool_calls'),
        ],
        () => {}
      );
    ...
    it('should handle empty chunks without choices', async () => {
      const scope = mockChatCompletion(
        [{}, { choices: [] }, chatTextChunk('Hello', 'stop')],
        () => {}
      );
